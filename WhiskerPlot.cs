...
{
    /// <summary>
    /// Visifire.Charts.WhiskerPlot
    /// </summary>
    internal class WhiskerPlot
    {
        /// <summary>
        ...
        {
            Brush oCRectfillColor = dataPoint.Color==null ? newBrush: dataPoint.Color;
            ...
            switch ((shape.Tag as ElementData).VisualElementName)
            {
                ...
                case "OcRect":
                    ...
                    shape.Stroke = GetRectangleBorderbrush(dataPoint, newBrush);
                    break;
                case "TopBevel":
        ...
        internal static Double CalculateDataPointWidth(Double width, Double height, Chart chart)
        {
            ...
            Double minDiffValue = chart.PlotDetails.GetMinOfMinDifferencesForXValue(RenderAs.Column, RenderAs.StackedColumn, RenderAs.StackedColumn100, RenderAs.Stock, RenderAs.WhiskerPlot);
        ...
        internal static void SetDataPointValues(DataPoint dataPoint, ref Double highY, ref Double lowY, ref Double openY, ref Double closeY)
        {
           // CandleStick.SetDataPointValues(dataPoint, ref highY, ref lowY, ref openY, ref closeY);
            ...
            {
                InterQuartileRange iqr = GetIQR(dataPoint.InternalYValues.ToList());
                openY = iqr.High75Quartile;
                closeY = iqr.Low25Quartile;
            }
            if (dataPoint.InternalYValues.Length >= 2)
            {
                lowY = dataPoint.InternalYValues.Min();
                highY = dataPoint.InternalYValues.Max();
            }
        }

        private static Brush GetRectangleBorderbrush(DataPoint dataPoint, Brush dataPointColor)
        {
            return (dataPoint.BorderColor == null) ?  dataPointColor : dataPoint.BorderColor;
        }
        ...
        internal static void ApplyOrRemoveBevel(DataPoint dataPoint, Double dataPointWidth)
        {
            ...
            if (dataPoint.Parent.Bevel && dataPointWidth > 8 && openCloseRect.Height > 6)
            {
                ...
                {
                    Brush color = (Brush)dataPoint.GetValue(DataPoint.ColorProperty);
        ...
        private static void UpdateYValueAndXValuePosition(DataPoint dataPoint, Double canvasWidth, Double canvasHeight, Double dataPointWidth)
        {
            ...
            double median;
            if (dataPoint.InternalYValues.Length % 2 == 0)
            {
                //even number, interpolate
                int low = dataPoint.InternalYValues.Length / 2;
                median = .5 * dataPoint.InternalYValues[low] + .5 * dataPoint.InternalYValues[low + 1];

            }
            else
            {
                int index = dataPoint.InternalYValues.Length / 2 + 1;
                median = dataPoint.InternalYValues[index];
            ...
            median = Graphics.ValueToPixelPosition(canvasHeight, 0, (double)plotGroup.AxisY.InternalAxisMinimum, (double)plotGroup.AxisY.InternalAxisMaximum, median);
            //non-sensical hold over from using candle plot as base of it
            dataPoint._isPriceUp = false;
            ...
            //Update media line 
            Line medianLine = dataPoint.Faces.VisualComponents[2] as Line;
            medianLine.X1 = 0;
            medianLine.X2 = dataPointVisual.Width;
            medianLine.Y1 = median-highY;
            medianLine.Y2 = median-highY;

            medianLine.Tag = new ElementData() { Element = dataPoint, VisualElementName = "MedianLine" };


            // Apply color for a CandleStick
            ApplyOrUpdateColorForAWhisker(dataPoint);
        ...
        private static void ApplyOrUpdateBorder(DataPoint dataPoint, Double dataPointWidth)
        {
            ...
            medianLine.StrokeThickness = GetStrokeThickness(dataPoint, dataPointWidth);
            medianLine.StrokeDashArray = Graphics.LineStyleToStrokeDashArray(dataPoint.BorderStyle.ToString());
            ...
            openCloseRect.Stroke = GetRectangleBorderbrush(dataPoint, (Brush)dataPoint.GetValue(DataPoint.ColorProperty));
        }
        ...
        /// <param name="dataPointWidth"></param>
        internal static void CreateOrUpdateAWhisker(DataPoint dataPoint, Canvas whiskerCanvas, Canvas labelCanvas, Double canvasWidth, Double canvasHeight, Double dataPointWidth)
        {
            Faces dpFaces = dataPoint.Faces;

            // Remove preexisting dataPoint visual and label visual
            if (dpFaces != null && dpFaces.Visual != null && whiskerCanvas == dpFaces.Visual.Parent)
            {
                whiskerCanvas.Children.Remove(dataPoint.Faces.Visual);
                //dpFaces = null;
            }
            ...
            //Create a media line
            Line medianLine = new Line() { Tag = tagElement };
            dataPoint.Faces.Parts.Add(medianLine);
            dataPoint.Faces.VisualComponents.Add(medianLine);
            dataPointVisual.Children.Add(medianLine);


            UpdateYValueAndXValuePosition(dataPoint, canvasWidth, canvasHeight, dataPointWidth);

            // Add dataPointVisual to seriesCanvas
            whiskerCanvas.Children.Add(dataPointVisual);
        ...
        /// </summary>
        /// <param name="dataPoint">DataPoint</param>
        internal static void ApplyOrUpdateColorForAWhisker(DataPoint dataPoint)
        {
            Faces dpFaces = dataPoint.Faces;
            Brush dataPointColor = (Brush)dataPoint.GetValue(DataPoint.ColorProperty);
            if (dataPointColor == null)
            {
                dataPointColor = dataPoint.Parent.Color;
            }
            ...
            openCloseRectColor = dataPointColor;
            ...
            if ((Boolean)dataPoint.LightingEnabled)
            {
                ...
                medianLine.Stroke = Graphics.GetLightingEnabledBrush(highLowLineColor, "Linear", null);
            }
            else
            {
                openCloseRect.Fill = openCloseRectColor;
                highLowLine.Stroke = highLowLineColor;
                medianLine.Stroke = highLowLineColor;
            }
        ...
        /// <summary>
        /// Get visual object for WhiskerPlot chart
        /// </summary>
        ...
        internal static Canvas GetVisualObjectForWhiskerPlot(Panel preExistingPanel, Double width, Double height, PlotDetails plotDetails, List<DataSeries> seriesList, Chart chart, Double plankDepth, bool animationEnabled)
        {
            // return new Canvas() { Background = Graphics.GetRandonColor() , Width = width, Height = height};

            if (Double.IsNaN(width) || Double.IsNaN(height) || width <= 0 || height <= 0) return null;

            Canvas visual, labelCanvas, whiskerPlotCanvas;

            RenderHelper.RepareCanvas4Drawing(preExistingPanel as Canvas, out visual, out labelCanvas, out whiskerPlotCanvas, width, height);
            ...
            foreach (DataSeries series in seriesList)
            {
                ...
                Faces dsFaces = new Faces() { Visual = whiskerPlotCanvas, LabelCanvas = labelCanvas };
                ...
                foreach (DataPoint dataPoint in viewPortDataPoints)
                    CreateOrUpdateAWhisker(dataPoint, whiskerPlotCanvas, labelCanvas, width, height, dataPointWidth);
                ...
                if (animationEnabled)
                {
                    ...
                    _tempDataSeries.Storyboard = AnimationHelper.ApplyOpacityAnimation(whiskerPlotCanvas, _tempDataSeries, _tempDataSeries.Storyboard, animationBeginTime, 1, 0, 1);
            ...
            if (preExistingPanel != null)
            {
                ...
                visual.Children.Add(whiskerPlotCanvas);
            }
            else
            {
        ...
        private static void UpdateDataPoint(DataPoint dataPoint, VcProperties property, object newValue, Boolean isAxisChanged)
        {
            ...
            {
                CreateOrUpdateAWhisker(dataPoint, dsFaces.Visual as Canvas, dsFaces.LabelCanvas, dsFaces.Visual.Width, dsFaces.Visual.Height, dataPointWidth);
                return;
            }
        ...
        public struct InterQuartileRange
        {
            public double Low25Quartile, High75Quartile;
        }

        /// <summary>
        /// Based on the R Function IQR in the quantile code 
        /// from the stats package
        /// </summary>
        /// <param name="Values"></param>
        /// <returns></returns>
        public static InterQuartileRange GetIQR(List<double> values)
        {
            double[] probs = new double[] { .25, .75 };
            values.Sort();
            int n = values.Count;
            var index = probs.Select(x =>  (n - 1) * x).ToArray();
            var low = index.Select(x => Math.Floor(x)).ToList();
            var high = index.Select(x => Math.Ceiling(x)).ToList();
            //get the low end of the rank order statistic
            var qs = low.Select(x => values[(int)x]).ToList();
            //now determine which values need to be interpolated
            //(essentially all of them as none are likely to be exactly 
            //at order statistic position)
            for (int i = 0; i < probs.Length; i++)
            {
                //interpolate if need be
                if (index[i] > low[i])
                {
                    double h = index[i] - low[i];
                    qs[i] = (1 - h) * qs[i] + h * values[(int)high[i]];
                }
            }
            return new InterQuartileRange() { Low25Quartile = qs[0], High75Quartile = qs[1] };
        }
